...
def cmp_age_group_sol(solutions):
    ...
    for i in range(8):
        axs[0, 0].plot(t, sol[0][:, i], label=labels[0][i], lw=1)
        axs[0, 1].plot(t, sol[1][:, i], label=labels[1][i], lw=1)
        axs[1, 0].plot(t, sol[2][:, i], label=labels[2][i], lw=1)
        axs[1, 1].plot(t, sol[3][:, i], label=labels[3][i], lw=1)
        axs[2, 0].plot(t, sol[4][:, i], label=labels[4][i], lw=1)
        axs[2, 1].plot(t, sol[5][:, i], label=labels[5][i], lw=1)
    for i in range(3):
        for j in range(2):
            axs[i, j].legend(
                fontsize="small",
